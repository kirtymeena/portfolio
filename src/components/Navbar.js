    ...
    <header>
      <nav className="navbar container">
        <div className="navbar__logo">
          <h1 className="letter-spacing"><Link to="/" className="link">Kirty</Link></h1>
        </div>
        {/* menu items for larger screen */}
        <div className="navbar__menu menu-items-lg">
          <div className="nav__item uppercase letter-sapcing"><Link onClick={(e)=>setActive(e.target.textContent)} className={`link underline-indicator ${active==="Home"?"active":""}` } to="#home" smooth>Home</Link></div>
          <div className="nav__item uppercase letter-sapcing"><Link onClick={(e)=>setActive(e.target.textContent)} className={`link underline-indicator ${active==="About"?"active":""}` } to="#about" smooth>About</Link></div>
          <div className="nav__item uppercase letter-sapcing"><Link onClick={(e)=>setActive(e.target.textContent)} className={`link underline-indicator ${active==="Project"?"active":""}` } to="/" smooth>Project</Link></div>
          <div className="nav__item uppercase letter-sapcing"><Link onClick={(e)=>setActive(e.target.textContent)} className={`link underline-indicator ${active==="Education"?"active":""}` } to="/" smooth>Education</Link></div>
        </div>
        ...
        <div className={`menu__link-sm ${open===true?"open":"close"}`}>
